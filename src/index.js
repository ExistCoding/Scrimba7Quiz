...
function App() {
    function restructureData(rawData) {
        ...
        function restructureQuestion(rawQuestion) {
            const indexCorrectAnswer = Math.floor(
                Math.random() * (rawQuestion.incorrect_answers.length + 1)
            );
    ...
    function handleAnswerButton(questionIndex, answerIndex) {
        setQuestions((prevQuestions) => {
            ...
            for (
                let i = 0;
                i < prevQuestions[questionIndex].answers.length;
                i++
            ) {
                if (i === answerIndex) {
                    newQuestions[questionIndex].answers[
                        ...
                        !prevQuestions[questionIndex].answers[answerIndex]
                            .isSelected;
                } else {
